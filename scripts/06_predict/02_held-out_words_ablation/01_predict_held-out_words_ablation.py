...
OUTPUT_DIR_BASE = PROJECT_ROOT / "output/prediction/03_held_out_words_cosine"
...
def _load_hidden_cosine(model_alias: str, prompt: str):
    ...
    hid_matrices = hid_data.get("cosine_similarity_matrices")
    if hid_matrices is None:
        return None, None
    if isinstance(hid_matrices, torch.Tensor):
        hid_matrices = hid_matrices.numpy()
    if hid_matrices.dtype == np.float64:
        hid_matrices = hid_matrices.astype(np.float32)
    return hid_matrices, hid_cues
...
def process_target_prompt(
    ...
    hid_target_mats, hid_target_vocab = _load_hidden_cosine(target_model, prompt)
    if hid_target_mats is None:
        logger.warning(f"  Missing target hidden cosine for {target_model}/{prompt}")
        return []
    ...
    for src in source_models:
        mats, vocab = _load_hidden_cosine(src, prompt)
        if mats is None:
            logger.warning(f"  Missing hidden cosine for source {src}/{prompt}, skipping")
            continue
        source_hid_mats.append(mats)
        source_vocab_maps.append({w: i for i, w in enumerate(vocab)})

    if not source_hid_mats:
        logger.warning("  No source models with hidden cosine matrices available.")
        return []
    ...
    hid_target_mats = hid_target_mats[:, idx_target][:, :, idx_target]
    ...
    for mats, w2i in zip(source_hid_mats, source_vocab_maps):
        idx_src = np.array([w2i[w] for w in common_words])
        mats = mats[:, idx_src][:, :, idx_src]
        source_hid_mats_sliced.append(mats)

    L_target = hid_target_mats.shape[0]
    if L_target <= 1:
        logger.warning("  Not enough layers after excluding layer 0.")
        return []

    expected_cross_dim = _expected_cross_dim(source_hid_mats_sliced)
    ...
    for seed_idx, seed in enumerate(args.seeds):
        ...
        logger.info(
            f"    Train words: {len(train_words):,} | Test words: {len(test_words):,}"
        )
...
def process_and_save(target_model: str, prompt: str, args, ft_bundle, bert_bundle, all_models: list[str]) -> bool:
    ...
    csv_path = out_dir / f"ridge_predict_held_out_words_cosine_{datetime_tag}.csv"
