#!/usr/bin/env python3
"""
Standalone script: Delta (ΔR²) summary heatmap.

Creates a heatmap with:
- rows: models
- columns: paradigms (FC, FA, FC+FA)
- square cells
- 2-decimal annotations

Input: `summary_prediction_analysis_*.csv` (model, prompt, delta_cross_fc_minus_cross,
delta_cross_fa_minus_cross, delta_cross_full_minus_cross).
...
INPUT_DIR = Path(
    "data/prediction/01_held_out_words/01_summary"
)
OUTPUT_DIR = Path(
    "output"
)
...
def plot_delta_heatmap(summary_df: pd.DataFrame, out_pdf: Path) -> None:
    required = {
        "model",
        "prompt",
        "delta_cross_fc_minus_cross",
        "delta_cross_fa_minus_cross",
        "delta_cross_full_minus_cross",
    }
    ...
    mean_deltas = mean_deltas.sort_values(
        "delta_cross_full_minus_cross", ascending=False
    ).reset_index(drop=True)
    models = mean_deltas["model"].tolist()

    hm_df = pd.DataFrame(
        {
            "FC+FA": mean_deltas["delta_cross_full_minus_cross"].to_numpy(dtype=float),
            "FC": mean_deltas["delta_cross_fc_minus_cross"].to_numpy(dtype=float),
            "FA": mean_deltas["delta_cross_fa_minus_cross"].to_numpy(dtype=float),
        },
        index=models,
    )
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=str,
        default=None,
        help="Output PDF path. Default: rr_delta_heatmap.pdf in the plots directory.",
    )
    args = ap.parse_args()
    ...
    if args.out_pdf is None:
        out_pdf = OUTPUT_DIR / "rr_delta_heatmap.pdf"
    else:
